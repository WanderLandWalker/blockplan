...
| 文件 | 适合谁 | 说明 |
|------|--------|------|
| `BlockPlan-0.2.9-windows-setup.exe` | Windows 常用用户 | 安装到系统里，后续从开始菜单或桌面启动 |
| `BlockPlan-0.2.9-windows-portable.exe` | 想免安装使用的人 | 双击即用，不写入安装目录 |
| `BlockPlan-0.2.9-android-debug.apk` | Android 手机用户 | 下载到手机后安装使用 |
| `BlockPlan-0.2.9-web.zip` | 只想用浏览器打开的人 | 解压后打开 `index.html` |
